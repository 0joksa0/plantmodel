...
#define TEST_RGR(hours, lambda_sni_v, lambda_sb_v, feedback_v, lit_val)                \
    ...
    {                                                                                  \
        ...
        input.photo.photoperiod = REAL(hours);                                               \
        input.carbohydrates.lambda_sni = REAL(lambda_sni_v);                                         \
        input.growth.lambda_sb = REAL(lambda_sb_v);                                           \
        input.photo.feedback_on_photosynthesis = REAL(feedback_v);                           \
        ...
        real_t rgr = compute_rgr(FW_start, input.growth.total_biomass, 29.0);                 \
        printf("%d model_RGR: %.6f, lit_RGR: %.6f\n",hours, rgr, REAL(lit_val));                \
        printf("%d leaf_start=%.6f, leaf_end=%.6f\n",hours, FW_start, input.growth.total_biomass); \
